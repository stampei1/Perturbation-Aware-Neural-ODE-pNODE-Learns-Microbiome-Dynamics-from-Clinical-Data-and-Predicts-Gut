...
def make_domination_predictions(model, test_df, index_of_taxa = 5, number_of_taxa = 'reduced', absolute_abundances = False):
        ...
        for patient_id in test_df['PatientID'].unique():
            ...
            for i in range(len(t_true)-1):
                ...
                if y0[index_of_taxa]>=0.3:
                    continue
                t_end = t_start + 7
                number_of_observations_in_the_next_7_days = ((t_start<t_true)*(t_true<=t_end)).sum()
                y_trues_in_the_next_7_days = y_trues[i+1:i+number_of_observations_in_the_next_7_days+1,:]
                # skip this i if there are no observations in the next 7 days from t_start
                if y_trues_in_the_next_7_days.numel()==0:
                    continue
                ...
                taxa_actual_trajectory = y_trues_in_the_next_7_days[:, index_of_taxa]
    ...
    # case that the model is working with absolute abundances
        ...
        for patient_id in test_df['PatientID'].unique():
            ...
            for i in range(len(t_true)-1):
                ...
                # now need to normalze y_trues_in_the_next_7_days
                positive_sums = y_trues_in_the_next_7_days.where(y_trues_in_the_next_7_days > 0, 0).sum(dim=1, keepdim=True)
                y_trues_in_the_next_7_days_normalized = y_trues_in_the_next_7_days / positive_sums.where(positive_sums != 0, 1)

                #pick just the taxa of interest
                taxa_pred_trajectory_normalized = y_pred_trajectory_normalized[:, index_of_taxa]
                taxa_actual_trajectory_normalized = y_trues_in_the_next_7_days_normalized[:, index_of_taxa]

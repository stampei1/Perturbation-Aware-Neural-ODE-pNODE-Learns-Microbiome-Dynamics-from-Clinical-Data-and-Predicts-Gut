...
def plot_coefficients(r, A, p, numspecies=5):
    ...
    ax.set_xlabel('Growth Rate (1/hour)')
    plt.savefig(f'./data/figure_eps_files/growth_coefs.eps', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    ax.set_yticks(range(numspecies), labels=[f's{i+1}' for i in reversed(range(numspecies))])
    ax.set_xlabel('Change in growth rate (1/hour) of each species in presence of perturbation')
    plt.title('Title')
    plt.savefig(f'./data/figure_eps_files/perturbation_coefs.eps', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.savefig(f'./data/figure_eps_files/interaction_coefs.eps', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
for i in range(5):
    ...
    ax.axvspan(start, stop,  color='lightsalmon')
